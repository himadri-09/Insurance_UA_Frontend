  ...
  return (
    <div className="card overflow-hidden">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center gap-3 px-5 py-4 border-b border-ink-800/50 hover:bg-ink-800/20 transition-colors">
        ...
        <h3 className="text-sm font-medium text-black flex-1 text-left">{title}</h3>
...
export default function UploadPage() {
  ...
  const [company, setCompany] = useState<CompanyFormData>({
    name: "", registration_number: "", description: "", entity_type: "",
    funding_stage: "", headcount: null, year_established: null,
    annual_revenue: null, annual_payroll: null, naics_code: "",
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const formData: SubmissionFormData = {
        company, business_description: businessDesc,
        property_description: propertyDesc, incidents,
      };
  ...
  // ── Result screen — Appetite only, no winnability/priority ──
  if (result) {
    return (
      <div className="max-w-2xl mx-auto">
        <div className="card p-8 text-center">
          ...
          <p className="text-sm text-ink-400 mb-1">
            {result.company?.name || "Submission"} — {result.line_of_business}
          </p>
          <p className="text-sm text-ink-500 mb-6">
            Completed in {result.processing_time_seconds.toFixed(1)}s
          </p>

          {/* Single appetite card only */}
          <div className="flex justify-center mb-8">
            <div className="card p-6 min-w-[160px]">
              ...
              <div className="text-xs text-ink-500 mt-1 capitalize">
                {result.appetite_assessment.status.replace(/_/g, " ")}
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-3 justify-center">
            <button
              onClick={() => router.push(`/dashboard/submissions/${result.submission_id}`)}
              className="btn-primary"
            >
              View Full Brief
            </button>
            <button
              onClick={() => { setResult(null); setFiles({}); setCurrentStage(0); setElapsed(0); }}
  ...
  if (processing) {
    return (
      <div className="max-w-2xl mx-auto">
        <div className="card p-8">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-ink-200">Processing submission...</span>
            </div>
          ...
          <div className="space-y-3">
            {STAGES.map((stage, i) => (
              <div key={stage.key} className="flex items-center gap-3">
                ...
                <span className={`text-sm ${i <= currentStage ? "text-ink-200" : "text-ink-600"}`}>
                  {stage.label}
                </span>
              </div>
            ))}
        ...
        {error && (
          <div className="card p-4 mt-4 border-accent-red/30 bg-accent-red/5 flex items-start gap-3">
            <AlertCircle className="w-4 h-4 text-accent-red mt-0.5" />
            <div>
              <p className="text-sm text-accent-red font-medium">Processing failed</p>
    ...
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-sm text-ink-400 mt-1">
          Submit commercial insurance documents and business details for AI triage
        </p>
      </div>
        ...
        <Section title="Company Details" icon={Building2}>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={company.name} onChange={(e) => setCompany({ ...company, name: e.target.value })} className="input-sm" placeholder="Acme Corp" />
            </div>
            <div>
              <label className="label">Registration / EIN</label>
              <input value={company.registration_number} onChange={(e) => setCompany({ ...company, registration_number: e.target.value })} className="input-sm" placeholder="XX-XXXXXXX" />
            </div>
            <div>
              <label className="label">Entity Type</label>
              <select value={company.entity_type} onChange={(e) => setCompany({ ...company, entity_type: e.target.value })} className="input-sm">
            ...
            <div>
              ...
              <input value={company.naics_code} onChange={(e) => setCompany({ ...company, naics_code: e.target.value })} className="input-sm" placeholder="541512" />
            </div>
            <div>
              <label className="label">Annual Revenue ($)</label>
              <input type="number" value={company.annual_revenue || ""} onChange={(e) => setCompany({ ...company, annual_revenue: e.target.value ? Number(e.target.value) : null })} className="input-sm" placeholder="5000000" />
            </div>
            <div>
              <label className="label">Annual Payroll ($)</label>
              <input type="number" value={company.annual_payroll || ""} onChange={(e) => setCompany({ ...company, annual_payroll: e.target.value ? Number(e.target.value) : null })} className="input-sm" placeholder="1000000" />
            </div>
            <div>
              <label className="label">Headcount</label>
              <input type="number" value={company.headcount || ""} onChange={(e) => setCompany({ ...company, headcount: e.target.value ? Number(e.target.value) : null })} className="input-sm" placeholder="25" />
            </div>
            <div>
              <label className="label">Year Established</label>
              <input type="number" value={company.year_established || ""} onChange={(e) => setCompany({ ...company, year_established: e.target.value ? Number(e.target.value) : null })} className="input-sm" placeholder="2010" />
            </div>
            <div>
              <label className="label">Funding Stage</label>
              <select value={company.funding_stage} onChange={(e) => setCompany({ ...company, funding_stage: e.target.value })} className="input-sm">
                <option value="">Select...</option>
          ...
          <div className="mt-4">
            ...
            <textarea value={businessDesc} onChange={(e) => setBusinessDesc(e.target.value)} className="input-sm h-20 resize-none" placeholder="Describe what the business does, operations, customers..." />
          </div>
        </Section>

        {/* Property Details */}
        <Section title="Property Details" icon={MapPin} defaultOpen={false}>
          <textarea value={propertyDesc} onChange={(e) => setPropertyDesc(e.target.value)} className="input-sm h-24 resize-none" placeholder="Locations, construction types, square footage, sprinkler status, alarm systems..." />
        </Section>
          ...
          {incidents.map((inc, i) => (
            <div key={i} className="grid grid-cols-4 gap-3 mb-3 items-end">
              ...
              <div className="flex gap-2">
                <div className="flex-1"><label className="label">Description</label><input value={inc.description} onChange={(e) => updateIncident(i, "description", e.target.value)} className="input-sm" placeholder="What happened" /></div>
                <button onClick={() => removeIncident(i)} className="p-2 text-ink-600 hover:text-accent-red"><X className="w-4 h-4" /></button>
              </div>
            </div>
          ))}
            ...
            {FILE_CATEGORIES.map((cat) => (
              <div key={cat.key} className="card p-4">
                <div className="flex items-center gap-2 mb-3">
                  <cat.icon className="w-4 h-4 text-ink-500" />
                  <span className="text-sm text-black font-medium">{cat.label}</span>
                </div>
                ...
                <label className="block cursor-pointer">
                  <div className="border border-dashed border-ink-700 rounded-lg p-3 text-center hover:border-ink-500 transition-colors">
                    <Upload className="w-4 h-4 text-ink-600 mx-auto mb-1" />
                    <span className="text-xs text-ink-500">Click to upload</span>
                  </div>
